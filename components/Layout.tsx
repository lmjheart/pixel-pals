...
interface LayoutProps {
  // Fixed: Changed React.NewNode to React.ReactNode which is the correct type for children
      ...
      <header className="bg-indigo-600 text-white p-4 shadow-lg sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <nav className="flex items-center space-x-2 sm:space-x-4">
            ...
            {currentUser ? (
              <div className="flex items-center bg-indigo-700/50 p-1 pl-3 sm:pl-4 rounded-full border border-indigo-400">
                <span className="text-yellow-400 font-black text-[10px] sm:text-xs hidden xs:inline">{currentUser}</span>
                <button 
                  onClick={onLogout}
